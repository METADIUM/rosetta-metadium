...
const (
	// NodeVersion is the version of gmet we are using.
	NodeVersion = "m0.9.7"

	// Blockchain is Ethereum.
	Blockchain string = "Metadium"
	...
	// MainnetGmetArguments are the arguments to start a mainnet gmet instance.
	MainnetGmetArguments = `--config=/app/metadium/gmet.toml --gcmode=archive --graphql`

	// IncludeMempoolCoins does not apply to rosetta-metadium as it is not UTXO-based.
	IncludeMempoolCoins = false
)

var (
	// TestnetGmetArguments are the arguments to start a testnet gmet instance.
	TestnetGmetArguments = fmt.Sprintf("%s --testnet", MainnetGmetArguments)

	// MainnetGenesisBlockIdentifier is the *types.BlockIdentifier
	// of the mainnet genesis block.
	MainnetGenesisBlockIdentifier = &types.BlockIdentifier{
		Hash:  params.MetadiumMainnetGenesisHash.Hex(),
	...
	TestnetGenesisBlockIdentifier = &types.BlockIdentifier{
		Hash:  params.MetadiumTestnetGenesisHash.Hex(),
		Index: GenesisBlockIndex,
	}
