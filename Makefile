...
build-release:
	...
	docker build -t rosetta-metadium:$(version) .;
	docker save rosetta-metadium:$(version) | gzip > rosetta-metadium-$(version).tar.gz;

update-tracer:
	curl https://raw.githubusercontent.com/ethereum/go-ethereum/master/eth/tracers/internal/tracers/call_tracer.js -o metadium/client/call_tracer.js
...
mocks:
	...
	mockery --dir metadium --all --case underscore --outpkg metadium --output mocks/metadium;
	${ADDLICENSE_INSTALL}
	${ADDLICENCE_SCRIPT} .;
